...
export default memo(function ModernTemplate({ cv }) {
  ...
  const fontStyle = {
    fontFamily: font_family === 'lora' ? "'Lora', serif"
      : font_family === 'serif' ? "'Source Serif 4', serif"
      : "'DM Sans', sans-serif",
  }
        ...
        <div className="space-y-2">
          {data.projects.map((proj, i) => (
            <div key={i}>
              ...
              {proj.description && (
                <div className="text-zinc-600 text-[10px] mt-0.5 [&_ul]:list-disc [&_ul]:pl-4">
        ...
        <div className="space-y-2">
          {(custom.entries || []).map((entry, i) => (
            <div key={i}>
              ...
              {entry.content && (
                <div className="text-zinc-600 text-[10px] [&_ul]:list-disc [&_ul]:pl-4">
  ...
  return (
    <div
      className="cv-preview bg-white text-zinc-800"
      style={{ ...fontStyle, fontSize: '11px', lineHeight: '1.5' }}
    >
      {/* Header */}
